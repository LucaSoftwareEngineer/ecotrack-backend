...
WORKDIR /src
COPY ["ecotrack-backend.csproj", "."]
RUN dotnet restore "./ecotrack-backend.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./ecotrack-backend.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./ecotrack-backend.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "ecotrack-backend.dll"]